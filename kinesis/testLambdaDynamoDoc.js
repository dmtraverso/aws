    ...
    */
    ...
    for (var user in counters) {
        ...
        for (var timestamp in counters[user]) {
            ...
            //console.log('Getting item with params', params);
            // Call DynamoDB
            dynamo.getItem(params, function(err, data) {
                if (err) {
                    ...
                    if (Object.keys(data).length !== 0) {
                        ...
                        if (counters[username][timestamp].length === 1) {
                            if (counters[username][timestamp][0].X === 0 && counters[username][timestamp][0].Y === 0) {
                                params.Item.count = 0;
                            }
                        }
