...
class RecordProcessor(kcl.RecordProcessorBase):
    ...
    def process_records(self, records, checkpointer):
        ...
        try:
            ...
            # Create dict to hold counters by user and timestamp
            counters = dict()
            f = open('/home/ec2-user/process.log', 'a')
            for record in records:
            	...
            	round_time = position['Time'] / 1000
                # Count number of events per second
            	if not username in counters:
                        # initialize dict for each user
            		counters[username] = {}
                if not round_time in counters[username]:
                        # initialize dict for each timestamp
                        counters[username][round_time] = []
                counters[username][round_time].append({'X': position['X'], 'Y': position['Y']})
            	# end of added logic
                if self.largest_seq == None or seq > self.largest_seq:
                    self.largest_seq = seq
            # Testing persistance in DynamoDB
            f.write('Counters: ' + str(counters) + '\n')
            # Loop through users
            for user, counter in counters.items():
                # Loop through timestamps
                f.write('Inside user: ' + str(user) + '\n')
                for timestamp, movs in counter.items():
                    # Update counters for each user in DynamoDB
                    ...
                    # If Item exists, we need to update it
                    if 'Item' in response:
                        item = response['Item']
                        # append movements to end of array
                        item['movs'].extend(movs)
                    else:
                        # create a new item
                        item = {
                            'username': username,
                    ...
                    # update movment counter
                    item['count'] = len(movs)
                    if item['count'] == 1:
                        # if there is only one movement, and it is == to X:0, Y:0
                        if movs[0]['X'] == 0 and movs[0]['Y'] == 0:
                            item['count'] = 0
                    # Put Item in DynamoDB
                    table.put_item(Item=item)
            # End of DynamoDB persistance
